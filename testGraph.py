...
def main():
	...
	tests = {
	"graph_tests" : [
			["python test_graph_update_big.py -t o+b -configfile verify_config.json {0} {1} -timeout 2400 -allocsize {2} -device {3}".format(runteststr, genresstr, str(args.mem_size), str(args.device)), ""],
			["python test_graph_update_big.py -t o+b -configfile verify_config_range.json {0} {1} -timeout 2400 -allocsize {2} -device {3}".format(runteststr, genresstr, str(args.mem_size), str(args.device)), ""]
		]
	}
