...
def main():
    ...
    if args.t:
        if any("c" in s for s in args.t):
            testcases["CUDA"] = os.path.join(
                build_path, str("c_graph_test") + executable_extension
            )
        if any("m" in s for s in args.t):
            testcases["mallocMC-FlatterScatter"] = os.path.join(
                build_path, str("m_graph_test_f") + executable_extension
            )
            testcases["mallocMC-Scatter"] = os.path.join(
                build_path, str("m_graph_test_s") + executable_extension
            )
        if any("g" in s for s in args.t):
            testcases["Gallatin"] = os.path.join(
                build_path, str("b_graph_test") + executable_extension
            )
        if any("x" in s for s in args.t):
            testcases["XMalloc"] = os.path.join(
                sync_build_path, str("x_graph_test") + executable_extension
            )
        if any("h" in s for s in args.t):
            testcases["Halloc"] = os.path.join(
                sync_build_path, str("h_graph_test") + executable_extension
            )
        if any("s" in s for s in args.t):
            testcases["ScatterAlloc"] = os.path.join(
                sync_build_path, str("s_graph_test") + executable_extension
            )
        if any("o" in s for s in args.t):
            testcases["Ouroboros-P-S"] = os.path.join(
                build_path, str("o_graph_test_p") + executable_extension
            )
            testcases["Ouroboros-P-VA"] = os.path.join(
                build_path, str("o_graph_test_vap") + executable_extension
            )
            # really slow
            testcases["Ouroboros-P-VL"] = os.path.join(
                build_path, str("o_graph_test_vlp") + executable_extension
            )
            testcases["Ouroboros-C-S"] = os.path.join(
                build_path, str("o_graph_test_c") + executable_extension
            )
            testcases["Ouroboros-C-VA"] = os.path.join(
                build_path, str("o_graph_test_vac") + executable_extension
            )
        ...
        if any("f" in s for s in args.t):
            testcases["FDGMalloc"] = os.path.join(
                sync_build_path, str("f_graph_test") + executable_extension
            )
        if any("r" in s for s in args.t):
            ...
            testcases["RegEff-AW"] = os.path.join(
                sync_build_path, str("r_graph_test_aw") + executable_extension
            )
    ...
    # ####################################################################################################
    if generate_results:
        generateResultsFromGraphUpdate(
            testcases, "results", "Graphs", "update", 2, ranged
        )
