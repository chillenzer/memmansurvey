...
#if TEST_FLATTERSCATTER
using MemoryManager = MemoryManagerMallocMC_FlatterScatter;
const std::string mem_name("mallocMC-FlatterScatter");
#elif TEST_SCATTER
using MemoryManager = MemoryManagerMallocMC_Scatter;
const std::string mem_name("mallocMC-Scatter");
#endif
...
void testrun(CSR<DataType> &input_graph, const json &config,
             const std::string &init_csv, const std::string &insert_csv,
             const std::string &delete_csv, int allocSizeinGB);
...
int main(int argc, char *argv[]) {
  ...
  printf("%sDynamic Graph Example\n%s", CLHighlight::break_line_blue_s,
         CLHighlight::break_line_blue_e);
  ...
  std::cout << " with " << csr_mat.rows << " vertices and " << csr_mat.nnz
            << " edges\n";
  std::cout << "--- " << mem_name << "---\n";
  if (writeMatrixStats) {
    ...
    results_init << " " << csr_mat.rows << ", " << csr_mat.nnz << ", "
    ...
    // #######################################################
    // #######################################################
    testrun<MemoryManager, DataType>(csr_mat, config, init_file, insert_file,
                                     delete_file, allocSizeinGB);
  }
...
void testrun(CSR<DataType> &input_graph, const json &config,
  ...
  const auto update_iterations{
      config.find("update_iterations").value().get<int>()};
  const auto batch_size{config.find("batch_size").value().get<int>()};
  const auto realistic_deletion{
      config.find("realistic_deletion").value().get<bool>()};
  ...
  // Instantiate graph
  DynGraph<VertexData, EdgeData, MemoryManagerType> dynamic_graph(
      allocationSize);
  ...
  for (auto round = 0; round < iterations; ++round) {
    ...
    if (verify_enabled) {
      ...
      std::string header =
          std::string("Initialization - ") + std::to_string(round);
      verification.verify(test_graph, header.c_str(),
                          OutputCodes::VERIFY_INITIALIZATION);
    }

    for (auto update_round = 0; update_round < update_iterations && !test_init;
         ++update_round, offset += range) {
      if (printDebugMessages)
        std::cout << "Update-Round: " << update_round + 1 << " / "
                  << update_iterations << std::endl;
      EdgeUpdateBatch<VertexData, EdgeData> insertion_updates(
          dynamic_graph.number_vertices);
      insertion_updates.generateEdgeUpdates(
          dynamic_graph.number_vertices, batch_size,
          (round * update_iterations) + update_round, range, offset);
      ...
      if (verify_enabled) {
        ...
        std::string header =
            std::string("Insertion - ") + std::to_string(update_round);
        verification.verify(test_graph, header.c_str(),
                            OutputCodes::VERIFY_INSERTION);
      }

      // tab out for update only
      //  if (realistic_deletion)
      //  {
      //  	EdgeUpdateBatch<VertexData, EdgeData>
      //  deletion_updates(dynamic_graph.number_vertices);
      //  	deletion_updates.generateEdgeUpdates(dynamic_graph, batch_size,
      //  (round * update_iterations) + update_round, range, offset);
      ...
      // 		std::string header = std::string("Deletion - ") +
      // std::to_string(update_round); 		verification.verify(test_graph,
      // header.c_str(), OutputCodes::VERIFY_DELETION);
      // 	}
      // }
      // else
      // {
  ...
  if (test_init) {
    results_init << init_result.mean_ << "," << init_result.std_dev_ << ","
                 << init_result.min_ << "," << init_result.max_ << ","
                 << init_result.median_ << "," << init_result.num_;
  } else {
    results_insert << insert_result.mean_ << "," << insert_result.std_dev_
                   << "," << insert_result.min_ << "," << insert_result.max_
                   << "," << insert_result.median_ << "," << insert_result.num_;
    // 	results_delete << delete_result.mean_ << "," << delete_result.std_dev_
    // << "," << delete_result.min_ << "," << delete_result.max_ << "," <<
    // delete_result.median_ << "," << delete_result.num_;
    //
  }

  if (printResults) {
    int width{10};
    std::cout << std::setw(width) << "-" << " | " << std::setw(width) << "iter"
              << " | " << std::setw(width) << "mean" << " | "
              << std::setw(width) << "std_dev" << " | " << std::setw(width)
              << "median\n";
