...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test graph edge updates for various frameworks"
    )
    parser.add_argument(
        "-t",
        type=str,
    ...
    parser.add_argument(
        "-configfile", type=str, help="Specify the config file: config.json"
    )
    parser.add_argument(
        "-graphstats",
        action="store_true",
    ...
    if args.t:
        ...
        if any("m" in s for s in args.t):
            testcases["mallocMC-FlatterScatter"] = os.path.join(
                build_path, str("m_graph_exp_test_f") + executable_extension
            )
            testcases["mallocMC-Scatter"] = os.path.join(
                build_path, str("m_graph_exp_test_s") + executable_extension
            )
        ...
        if any("x" in s for s in args.t):
            testcases["XMalloc"] = os.path.join(
                sync_build_path, str("x_graph_exp_test") + executable_extension
            )
        if any("h" in s for s in args.t):
            testcases["Halloc"] = os.path.join(
                sync_build_path, str("h_graph_exp_test") + executable_extension
            )
        if any("s" in s for s in args.t):
            testcases["ScatterAlloc"] = os.path.join(
                sync_build_path, str("s_graph_exp_test") + executable_extension
            )
        if any("o" in s for s in args.t):
            testcases["Ouroboros-P-S"] = os.path.join(
                build_path, str("o_graph_exp_test_p") + executable_extension
            )
            testcases["Ouroboros-P-VA"] = os.path.join(
                build_path, str("o_graph_exp_test_vap") + executable_extension
            )
            # really slow
            testcases["Ouroboros-P-VL"] = os.path.join(
                build_path, str("o_graph_exp_test_vlp") + executable_extension
            )
            testcases["Ouroboros-C-S"] = os.path.join(
                build_path, str("o_graph_exp_test_c") + executable_extension
            )
            testcases["Ouroboros-C-VA"] = os.path.join(
                build_path, str("o_graph_exp_test_vac") + executable_extension
            )
        ...
        if any("f" in s for s in args.t):
            testcases["FDGMalloc"] = os.path.join(
                sync_build_path, str("f_graph_exp_test") + executable_extension
            )
        if any("r" in s for s in args.t):
            ...
            testcases["RegEff-AW"] = os.path.join(
                sync_build_path, str("r_graph_exp_test_aw") + executable_extension
            )
    ...
    if args.graphstats:
        ...
        for graph in graphs:
            ...
            run_config = (
                config_file + " " + path + graph + " " + str(1) + " " + csv_path
            )
            executecommand = "{0} {1}".format(
                os.path.join(build_path, str("c_graph_test") + executable_extension),
                run_config,
            )
            print(
                "#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#"
    ...
    if run_testcases:
        for name, executable in testcases.items():
            ...
            if "range" in config_file:
                suffix = "_range.csv"
            csv_path_insert = "results/graph_exp_update_" + name + "_insert" + suffix
            csv_path_delete = "results/graph_exp_update_" + name + "_delete" + suffix
    ...
    # ####################################################################################################
    if generate_results:
        generateResultsFromGraphUpdateExp(
            testcases, "results", "Graphs", "exp_update", 3, ranged
        )
