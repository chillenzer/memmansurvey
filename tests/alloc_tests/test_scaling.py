...
def main():
    ...
    parser.add_argument(
        "-byterange", type=str, help="Specify Allocation Range, e.g. 16-8192"
    )
    parser.add_argument(
        "-threadrange",
        type=str,
    ...
    if args.t:
        if any("c" in s for s in args.t):
            testcases["CUDA"] = os.path.join(
                build_path, str("c_alloc_test") + executable_extension
            )
        if any("m" in s for s in args.t):
            testcases["mallocMC-FlatterScatter"] = os.path.join(
                build_path, str("m_alloc_test_f") + executable_extension
            )
            testcases["mallocMC-Scatter"] = os.path.join(
                build_path, str("m_alloc_test_s") + executable_extension
            )
        if any("g" in s for s in args.t):
            testcases["Gallatin"] = os.path.join(
                build_path, str("b_alloc_test") + executable_extension
            )
        if any("x" in s for s in args.t):
            testcases["XMalloc"] = os.path.join(
                build_path, str("x_alloc_test") + executable_extension
            )
        ...
        if any("o" in s for s in args.t):
            ...
            testcases["Ouroboros-P-VA"] = os.path.join(
                build_path, str("o_alloc_test_vap") + executable_extension
            )
            testcases["Ouroboros-P-VL"] = os.path.join(
                build_path, str("o_alloc_test_vlp") + executable_extension
            )
            testcases["Ouroboros-C-S"] = os.path.join(
                build_path, str("o_alloc_test_c") + executable_extension
            )
            testcases["Ouroboros-C-VA"] = os.path.join(
                build_path, str("o_alloc_test_vac") + executable_extension
            )
            testcases["Ouroboros-C-VL"] = os.path.join(
                build_path, str("o_alloc_test_vlc") + executable_extension
            )
        if any("f" in s for s in args.t):
            testcases["FDGMalloc"] = os.path.join(
                build_path, str("f_alloc_test") + executable_extension
            )
        if any("r" in s for s in args.t):
            # testcases["RegEff-A"] = os.path.join(build_path, str("r_alloc_test_a") + executable_extension)
            testcases["RegEff-AW"] = os.path.join(
                build_path, str("r_alloc_test_aw") + executable_extension
            )
            testcases["RegEff-C"] = os.path.join(
                build_path, str("r_alloc_test_c") + executable_extension
            )
            testcases["RegEff-CF"] = os.path.join(
                build_path, str("r_alloc_test_cf") + executable_extension
            )
            testcases["RegEff-CM"] = os.path.join(
                build_path, str("r_alloc_test_cm") + executable_extension
            )
            testcases["RegEff-CFM"] = os.path.join(
                build_path, str("r_alloc_test_cfm") + executable_extension
            )
