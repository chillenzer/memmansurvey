...
def main():
    ...
    parser.add_argument(
        "-t",
        type=str,
        help="Specify which frameworks to test, separated by +, e.g. o+s+h+c+f+r+x ---> c : cuda | s : scatteralloc | h : halloc | o : ouroboros | f : fdgmalloc | r : register-efficient | x : xmalloc",
    )
    parser.add_argument("-num", type=int, help="How many allocations to perform")
    parser.add_argument(
        "-range",
        type=str,
        help="Specify Allocation Range from which these allocations come from, e.g. 4-1024",
    )
    ...
    parser.add_argument(
        "-warp", action="store_true", default=False, help="Start testcases warp-based"
    )
    parser.add_argument(
        "-devmeasure",
        action="store_true",
    ...
    if args.t:
        if any("c" in s for s in args.t):
            testcases["CUDA"] = os.path.join(
                build_path, str("c_mixed_alloc_test") + executable_extension
            )
        if any("m" in s for s in args.t):
            testcases["mallocMC-FlatterScatter"] = os.path.join(
                build_path, str("m_mixed_alloc_test_f") + executable_extension
            )
            testcases["mallocMC-Scatter"] = os.path.join(
                build_path, str("m_mixed_alloc_test_s") + executable_extension
            )
        if any("g" in s for s in args.t):
            testcases["Gallatin"] = os.path.join(
                build_path, str("b_mixed_alloc_test") + executable_extension
            )
        if any("x" in s for s in args.t):
            testcases["XMalloc"] = os.path.join(
                build_path, str("x_mixed_alloc_test") + executable_extension
            )
        if any("h" in s for s in args.t):
            testcases["Halloc"] = os.path.join(
                build_path, str("h_mixed_alloc_test") + executable_extension
            )
        if any("s" in s for s in args.t):
            testcases["ScatterAlloc"] = os.path.join(
                build_path, str("s_mixed_alloc_test") + executable_extension
            )
        if any("o" in s for s in args.t):
            testcases["Ouroboros-P-S"] = os.path.join(
                build_path, str("o_mixed_alloc_test_p") + executable_extension
            )
            testcases["Ouroboros-P-VA"] = os.path.join(
                build_path, str("o_mixed_alloc_test_vap") + executable_extension
            )
            testcases["Ouroboros-P-VL"] = os.path.join(
                build_path, str("o_mixed_alloc_test_vlp") + executable_extension
            )
            testcases["Ouroboros-C-S"] = os.path.join(
                build_path, str("o_mixed_alloc_test_c") + executable_extension
            )
            testcases["Ouroboros-C-VA"] = os.path.join(
                build_path, str("o_mixed_alloc_test_vac") + executable_extension
            )
            testcases["Ouroboros-C-VL"] = os.path.join(
                build_path, str("o_mixed_alloc_test_vlc") + executable_extension
            )
        if any("f" in s for s in args.t):
            testcases["FDGMalloc"] = os.path.join(
                build_path, str("f_mixed_alloc_test") + executable_extension
            )
        if any("r" in s for s in args.t):
            ...
            testcases["RegEff-AW"] = os.path.join(
                build_path, str("r_mixed_alloc_test_aw") + executable_extension
            )
            testcases["RegEff-C"] = os.path.join(
                build_path, str("r_mixed_alloc_test_c") + executable_extension
            )
            testcases["RegEff-CF"] = os.path.join(
                build_path, str("r_mixed_alloc_test_cf") + executable_extension
            )
            testcases["RegEff-CM"] = os.path.join(
                build_path, str("r_mixed_alloc_test_cm") + executable_extension
            )
            testcases["RegEff-CFM"] = os.path.join(
                build_path, str("r_mixed_alloc_test_cfm") + executable_extension
            )
    ...
    if run_testcases:
        ...
        for name, path in testcases.items():
            csv_path_alloc = (
                "results/mixed_performance/perf_mixed_alloc_"
                + name
                + "_"
                + str(num_allocations)
                + "_"
            ...
            csv_path_free = (
                "results/mixed_performance/perf_mixed_free_"
                + name
                + "_"
                + str(num_allocations)
                + "_"
    ...
    if generate_results:
        ...
        generateResultsFromFileAllocation(
            testcases,
