...
def main():
	...
	if os.name == 'nt': # If on Windows
		...
		sync_build_path = os.path.join("build", "Release")
	else:
		build_path = "build/"
	...
	if(args.t):
		if any("c" in s for s in args.t):
			testcases["CUDA"] = os.path.join(build_path, str("c_frag_mixed") + executable_extension)
		if any("g" in s for s in args.t):
			testcases["Gallatin"] = os.path.join(build_path, str("b_frag_mixed") + executable_extension)
		if any("x" in s for s in args.t):
			testcases["XMalloc"] = os.path.join(sync_build_path, str("x_frag_mixed") + executable_extension)
		if any("h" in s for s in args.t):
			testcases["Halloc"] = os.path.join(sync_build_path, str("h_frag_mixed") + executable_extension)
		if any("s" in s for s in args.t):
			testcases["ScatterAlloc"] = os.path.join(sync_build_path, str("s_frag_mixed") + executable_extension)
		if any("o" in s for s in args.t):
			testcases["Ouroboros-P-S"] = os.path.join(build_path, str("o_frag_mixed_p") + executable_extension)
			testcases["Ouroboros-P-VA"] = os.path.join(build_path, str("o_frag_mixed_vap") + executable_extension)
			testcases["Ouroboros-P-VL"] = os.path.join(build_path, str("o_frag_mixed_vlp") + executable_extension)
			testcases["Ouroboros-C-S"] = os.path.join(build_path, str("o_frag_mixed_c") + executable_extension)
			#C-VA ruins everything for everyone.
			#testcases["Ouroboros-C-VA"] = os.path.join(build_path, str("o_frag_mixed_vac") + executable_extension)
			testcases["Ouroboros-C-VL"] = os.path.join(build_path, str("o_frag_mixed_vlc") + executable_extension)
		if any("f" in s for s in args.t):
			testcases["FDGMalloc"] = os.path.join(sync_build_path, str("f_frag_mixed") + executable_extension)
		if any("r" in s for s in args.t):
			testcases["RegEff-A"] = os.path.join(sync_build_path, str("r_frag_mixed_a") + executable_extension)
			testcases["RegEff-AW"] = os.path.join(sync_build_path, str("r_frag_mixed_aw") + executable_extension)
			testcases["RegEff-C"] = os.path.join(sync_build_path, str("r_frag_mixed_c") + executable_extension)
			testcases["RegEff-CF"] = os.path.join(sync_build_path, str("r_frag_mixed_cf") + executable_extension)
			testcases["RegEff-CM"] = os.path.join(sync_build_path, str("r_frag_mixed_cm") + executable_extension)
			testcases["RegEff-CFM"] = os.path.join(sync_build_path, str("r_frag_mixed_cfm") + executable_extension)
	...
	if generate_results:
		...
		generateResultsFromFileFragmentationMixed("results", num_allocations, smallest_allocation_size, largest_allocation_size, "Bytes", 2, num_iterations)
