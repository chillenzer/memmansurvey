...
def main():
    ...
    parser.add_argument(
        "-timeout",
        type=int,
        help="Timeout Value in Seconds, per round, process will be killed after as many seconds",
    )
    ...
    if args.t:
        ...
        if any("o" in s for s in args.t):
            ...
            testcases["Ouroboros-C-VA"] = os.path.join(
                build_path, str("o_frag_test_vac") + executable_extension
            )
        ...
        for name, executable in testcases.items():
            ...
            while allocation_size <= largest_allocation_size:
                ...
                if process_killed:
                    ...
                    with open(csv_path, "a", newline="") as csv_file:
                        csv_file.write(
                            "-------------------> Ran longer than " + str(time_out_val)
                        )
                else:
                    print("Success!")
                allocation_size *= 2
