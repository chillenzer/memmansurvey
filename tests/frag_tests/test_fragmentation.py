...
def main():
    ...
    if args.t:
        if any("c" in s for s in args.t):
            testcases["CUDA"] = os.path.join(
                build_path, str("c_frag_test") + executable_extension
            )
        if any("m" in s for s in args.t):
            testcases["mallocMC-FlatterScatter"] = os.path.join(
                build_path, str("m_frag_test_f") + executable_extension
            )
            testcases["mallocMC-Scatter"] = os.path.join(
                build_path, str("m_frag_test_s") + executable_extension
            )
        if any("g" in s for s in args.t):
            testcases["Gallatin"] = os.path.join(
                build_path, str("b_frag_test") + executable_extension
            )
        if any("x" in s for s in args.t):
            testcases["XMalloc"] = os.path.join(
                sync_build_path, str("x_frag_test") + executable_extension
            )
        if any("h" in s for s in args.t):
            testcases["Halloc"] = os.path.join(
                sync_build_path, str("h_frag_test") + executable_extension
            )
        if any("s" in s for s in args.t):
            testcases["ScatterAlloc"] = os.path.join(
                sync_build_path, str("s_frag_test") + executable_extension
            )
        if any("o" in s for s in args.t):
            testcases["Ouroboros-P-S"] = os.path.join(
                build_path, str("o_frag_test_p") + executable_extension
            )
            testcases["Ouroboros-P-VA"] = os.path.join(
                build_path, str("o_frag_test_vap") + executable_extension
            )
            testcases["Ouroboros-P-VL"] = os.path.join(
                build_path, str("o_frag_test_vlp") + executable_extension
            )
            testcases["Ouroboros-C-S"] = os.path.join(
                build_path, str("o_frag_test_c") + executable_extension
            )
            ...
            testcases["Ouroboros-C-VL"] = os.path.join(
                build_path, str("o_frag_test_vlc") + executable_extension
            )
        if any("f" in s for s in args.t):
            testcases["FDGMalloc"] = os.path.join(
                sync_build_path, str("f_frag_test") + executable_extension
            )
        if any("r" in s for s in args.t):
            testcases["RegEff-A"] = os.path.join(
                sync_build_path, str("r_frag_test_a") + executable_extension
            )
            testcases["RegEff-AW"] = os.path.join(
                sync_build_path, str("r_frag_test_aw") + executable_extension
            )
            testcases["RegEff-C"] = os.path.join(
                sync_build_path, str("r_frag_test_c") + executable_extension
            )
            testcases["RegEff-CF"] = os.path.join(
                sync_build_path, str("r_frag_test_cf") + executable_extension
            )
            testcases["RegEff-CM"] = os.path.join(
                sync_build_path, str("r_frag_test_cm") + executable_extension
            )
            testcases["RegEff-CFM"] = os.path.join(
                sync_build_path, str("r_frag_test_cfm") + executable_extension
            )
    ...
    if generate_results:
        ...
        generateResultsFromFileFragmentation(
            "results",
            num_allocations,
