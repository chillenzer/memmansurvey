...
def main():
	...
	if(args.t):
		...
		if any("g" in s for s in args.t):
			testcases["Gallatin"] = os.path.join(build_path, str("b_synth_test") + executable_extension)
		...
		if any("b" in s for s in args.t):
			testcases["Baseline"] = os.path.join(sync_build_path, str("base_synth_test") + executable_extension)
