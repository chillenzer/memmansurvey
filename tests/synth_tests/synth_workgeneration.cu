...
__global__ void d_baseline_requirements(const int *__restrict allocation_size,
                                        int num_allocations,
                                        int *__restrict requirements) {
...
__global__ void d_write_assignment(int *__restrict assignment,
                                   int *__restrict pos, int num_allocations,
                                   const int *__restrict requirements) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid >= num_allocations)
    return;
...
__global__ void d_write(const int *__restrict assignment,
                        const int *__restrict position,
                        const int *__restrict requirements,
                        int **__restrict verification_ptr, int num_values) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid >= num_values)
    return;
...
__global__ void d_baseline_write(const int *__restrict requirements,
                                 int num_allocations,
                                 int *__restrict storage_area,
...
__global__ void d_memorymanager_alloc(MemoryManager mm,
                                      const int *__restrict allocation_size,
                                      int num_allocations,
                                      int **__restrict verification_ptr) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid >= num_allocations)
  ...
  verification_ptr[tid] =
      reinterpret_cast<int *>(mm.malloc(allocation_size[tid]));
}

__global__ void d_memorymanager_free(MemoryManager mm,
                                     const int *__restrict allocation_size,
                                     int num_allocations,
                                     int **__restrict verification_ptr) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
int main(int argc, char *argv[]) {
  ...
  allocation_size_range_lower =
      Utils::alignment(allocation_size_range_lower, sizeof(int));
  allocation_size_range_upper =
      Utils::alignment(allocation_size_range_upper, sizeof(int));
  std::cout << "Number of Allocations: " << num_allocations
            << " | Allocation Range: " << allocation_size_range_lower << " - "
            << allocation_size_range_upper << std::endl;
  ...
  if (testWrite) {
    for (auto i = 0; i < num_iterations; ++i) {
      std::mt19937 gen(
          i); // Standard mersenne_twister_engine seeded with iteration
      ...
      CHECK_ERROR(
          cudaMalloc(&requirements, (num_allocations + 1) * sizeof(int)));

      d_baseline_requirements<<<gridSize, blockSize>>>(
          d_allocation_sizes.get(), num_allocations, requirements);
      ...
      int num_items{0};
      CHECK_ERROR(cudaMemcpy(&num_items, requirements + num_allocations,
      ...
      d_baseline_write<<<gridSize, blockSize>>>(requirements, num_allocations,
                                                storage_area,
                                                d_verification_ptrs.get());

#else
      d_memorymanager_alloc<<<gridSize, blockSize>>>(
          memory_manager, d_allocation_sizes.get(), num_allocations,
          d_verification_ptrs.get());
#endif
      ...
      d_write_assignment<<<gridSize, blockSize>>>(
          d_assignment.get(), d_pos.get(), num_allocations, requirements);

      timing.startMeasurement();
      gridSize = Utils::divup(num_integers, blockSize);
      for (auto j = 0; j < 100; ++j)
        d_write<<<gridSize, blockSize>>>(
            d_assignment.get(), d_pos.get(), requirements,
            d_verification_ptrs.get(), num_integers);
...
#else
      d_memorymanager_free<<<gridSize, blockSize>>>(
          memory_manager, d_allocation_sizes.get(), num_allocations,
          d_verification_ptrs.get());
#endif
  ...
  results << result.mean_ << "," << result.std_dev_ << "," << result.min_ << ","
          << result.max_ << "," << result.median_;
